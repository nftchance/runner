...
from runner.routing import application
...
class TestWebSocket:
    ...
    #     settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
    #     communicator = WebsocketCommunicator(application=application, path="/runner/")
    #     connected, _ = await communicator.connect()
    ...
    #     _, access = await create_user("test.user@example.com", "pAssw0rd")
    #     communicator = WebsocketCommunicator(
    #         application=application, path=f"/runner/?token={access}"
    #     )
